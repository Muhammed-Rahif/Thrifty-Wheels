...
module.exports = {
  adminLogin: (reqData) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (existAdmin) {
            ...
            bcrypt.compare(reqData.code, existAdmin.code).then((status) => {
              if (status) {
                bcrypt.compare(reqData.ip, existAdmin.ip).then((status) => {
                  if (status) {
                    bcrypt
                      .compare(reqData.password, existAdmin.password)
                      .then((status) => {
                        if (status) {
                          resolve({ loginStatus: true });
                        } else {
                          resolve({
                            loginStatus: "Incorrect password typed..!",
                          });
                        }
                      });
                  } else {
        ...
        let adminDetails = {
          name: "Rahif",
          email: "rahif@gmail.com",
          password:
            "$2b$10$7IfVp8ZxhwpA9EWJ1Faoi.xQj0s0P8qDsz9seWV6Aq2eps24Wr01C",
                  ...
                  if (status) {
                    bcrypt.compare(reqData.ip, existAdmin.ip).then((status) => {
                      if (status) {
                        ...
                        resolve({
                          loginStatus: "Incorrect secret ip typed..!",
                        });
                      }
                    });
                  } else {
  ...
  addPost: (postData) => {
    return new Promise((resolve, reject) => {
      db.get()
        .collection(collections.POSTS_COLLECTION)
        .insertOne(postData)
        .then(() => {
          resolve();
        });
